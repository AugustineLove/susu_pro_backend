...
export const getAccountsByCustomer = async (req, res) => {
  ...
  `SELECT
     -- Deposits
     COALESCE(SUM(CASE 
       WHEN t.type = 'deposit' AND t.is_deleted = false AND status IN ('approved', 'completed') THEN t.amount 
       ELSE 0 END), 0) AS total_deposits,

     -- Withdrawals
     COALESCE(SUM(CASE 
       WHEN t.type = 'withdrawal' AND t.is_deleted = false AND status IN ('approved', 'completed') THEN t.amount 
       ELSE 0 END), 0) AS total_withdrawals,

     -- Transfer In
     COALESCE(SUM(CASE 
       WHEN t.type = 'transfer_in' AND t.is_deleted = false AND status IN ('approved', 'completed') THEN t.amount 
       ELSE 0 END), 0) AS total_transfer_ins,

     -- Transfer Out
     COALESCE(SUM(CASE 
       WHEN t.type = 'transfer_out' AND t.is_deleted = false AND status IN ('approved', 'completed') THEN t.amount 
